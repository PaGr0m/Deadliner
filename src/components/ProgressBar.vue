<template>
    <div>
        <div v-for="bar in bars" :key="bar.label" class="row mb-1 mr-5">
            ...
            <div class="col-sm-2 "><b>{{ bar.dead_date.getDate() }}:{{ bar.dead_date.getMonth() + 1}}:{{ bar.dead_date.getFullYear() }} | {{ bar.dead_date.getHours() }}:{{ bar.dead_date.getMinutes() }}</b></div>
            ...
            <div class="col-sm-2 "><b>{{ bar.add_date.getDate() }}:{{ bar.add_date.getMonth() + 1}}:{{ bar.add_date.getFullYear() }} | {{ bar.add_date.getHours() }}:{{ bar.add_date.getMinutes() }}</b></div>
        </div>
    </div>
</template>
    ...
    export default {
        data() {
            return {
                ...
                bars: [
                    { variant: "danger", value: 0, add_date: new Date('October 28, 2019 03:24:00'), dead_date :new Date('October 28, 2019 03:24:00'), name: 'Unix/Python' },
